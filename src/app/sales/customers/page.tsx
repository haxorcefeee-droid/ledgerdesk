...
export default async function SalesCustomersPage({
  ...
  const parties = await db.all<{
    ...
    `SELECT * FROM parties WHERE business_id = ? AND kind = 'customer'
  ...
  return (
    <AppShell current="customers">
      ...
      <SearchForm placeholder="Search customers" />
      ...
      <DataTable headers={["Name", "Email", "Credit limit", "Currency", "Statement"]}>
        {parties.map((party) => (
          <tr key={party.id} className="border-t border-[var(--line)]">
            ...
            <td className="px-4 py-3 sans">{formatMoney(party.credit_limit_cents, party.currency || tenant.business.currency)}</td>
            <td className="px-4 py-3">{party.currency}</td>
